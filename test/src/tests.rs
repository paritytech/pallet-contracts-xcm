use crate::contract_utils::{read_contract, Contract, Spec};
use mock_network::{
...
fn instantiate_test_contract() -> (Spec, AccountId) {
    let Contract { source, spec } = read_contract();
    ...
    let contract_addr = ParaA::execute_with(|| {
        ParachainContracts::bare_instantiate(
            ...
            Weight::MAX,
            None,
            Code::Upload(source.wasm),
            spec.constructors["new"].selector.encode(),
            vec![],
...
fn test_xcm_execute() {
    ...
    let (spec, contract_addr) = instantiate_test_contract();
    ...
    ParaA::execute_with(|| {
        ...
        assert_ok!(
            ParachainContracts::bare_call(
                ...
                Weight::MAX,
                None,
                (spec.messages["xcm_transfer"].selector.clone(), message).encode(),
                DebugInfo::UnsafeDebug,
    ...
    ParaA::execute_with(|| {
        ...
        assert_ok!(
            ParachainContracts::bare_call(
                ...
                None,
                (
                    spec.messages["xcm_send"].selector.clone(),
...
fn test_xcm_new_query() {
    ...
    ParaA::execute_with(|| {
        ...
        let exec = ParachainContracts::bare_call(
            ...
            None,
            (
                spec.messages["xcm_new_query"].selector.clone(),
                timeout,
                match_querier,
            )
                .encode(),
    ...
    ParaA::execute_with(|| {
        ...
        let call = |query_id| {
            let exec = ParachainContracts::bare_call(
                ...
                None,
                (
                    spec.messages["xcm_take_response"].selector.clone(),
                    query_id,
                )
                    .encode(),
                DebugInfo::UnsafeDebug,
