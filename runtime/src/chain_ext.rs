		...
		<T as SysConfig>::AccountId: AsRef<[u8; 32]>,
	{
			...
			<E::T as SysConfig>::AccountId: UncheckedFrom<<E::T as SysConfig>::Hash> + AsRef<[u8]>,
		{
			match Command::try_from(env.func_id()).map_err(|_| PalletError::<T>::InvalidCommand)? {
				...
				Command::Execute => {
 					let input =
						self.prepared_execute.as_ref().take().ok_or(PalletError::<T>::PreparationMissing)?; 
				...
				Command::Send => {
					let input =
						self.validated_send.as_ref().take().ok_or(PalletError::<T>::PreparationMissing)?;
				...
				Command::NewQuery => {
					...
					let query_id: u64 =
						pallet_xcm::Pallet::<T>::new_query(location, Bounded::max_value(),location).into();
